...
def check_rule_by_supplier_name(collection, supplier_name):
    ...
    for result in results:
        rules_list.append(result.get("rule", []))  # 取得"rules"欄位，如果不存在則回傳null list
    return rules_list
...
# 提出問題並獲取答案
question = "出版商是哪間公司？"
new_supplier = llm_chain.run(question=question, context=texts[0].page_content)  # 使用第一個 chunk 作為 context
